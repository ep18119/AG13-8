		...
		return ((event.M === 1) || (model.g.q || (n.e === 1))) ? model : A3(
			...
			A2(
				...
				A3(
					...
					$elm$core$Basics$add,
					A3($author$project$AG2$pairMap2, $elm$core$Basics$sub, event.T, event.ag),
					_Utils_Tuple2(30, 30)),
				A3($author$project$AG2$ASTne, n, b, r)),
			true,
			model);
...
var $author$project$AG2$letMeRoot = F3(
	function (_v0, event, model) {
		...
		if ((event.M !== 1) || model.g.q) {
			...
			var xy = A3($author$project$AG2$pairMap2, $elm$core$Basics$sub, event.T, event.ag);
			var t = node.a;
			var f = F4(
				function (xy0, n, b, r) {
		...
		return (!model.g.q) ? model : _Utils_update(
			model,
			{
				g: {
					a: model.g.a,
					C: model.g.C,
					I: A3($author$project$AG2$pairMap2, $elm$core$Basics$sub, event.B, model.g.D),
					q: model.g.q,
					D: model.g.D
				}
			});
		...
		return (event.M !== 1) ? model : _Utils_update(
			model,
			{
				g: {
					...
					I: xy,
					q: true,
					D: A3($author$project$AG2$pairMap2, $elm$core$Basics$sub, event.B, xy)
				}
			});
	});
				...
				return _Utils_Tuple2(
					A2(
						...
						_Utils_update(
							model,
							{
								B: A3($author$project$AG2$pairMap2, $elm$core$Basics$sub, event.T, event.B)
							})),
					$elm$core$Platform$Cmd$none);
			case 3:
				var astxy = msg.a;
				...
				return _Utils_Tuple2(
					...
					$author$project$AG2$sendMes(
						A2(
							...
							_List_fromArray(
								[
									$elm$json$Json$Encode$int(model.z + 1),
...
var $mpizenberg$elm_pointer_events$Html$Events$Extra$Mouse$Event = F6(
	function (keys, button, clientPos, offsetPos, pagePos, screenPos) {
		return {M: button, B: clientPos, bg: keys, ag: offsetPos, T: pagePos, bt: screenPos};
	});
...
var $author$project$AG2$viewASTxy = F3(
	function (model, te, _v0) {
		...
		var x = _v3.a;
		var y = _v3.b;
		return ((!te) && (!_Utils_eq(model.z, n.a))) ? A3($elm$html$Html$Keyed$node, 'div', _List_Nil, _List_Nil) : ((te && (!_Utils_eq(model.S, n.a))) ? A3($elm$html$Html$Keyed$node, 'div', _List_Nil, _List_Nil) : A3(
			$elm$html$Html$Keyed$node,
			'div',
			_List_fromArray(
				[
				...
				A2(
					...
					F2(
						function (index, tab) {
							return _Utils_Tuple2(
								$elm$core$String$fromInt(index),
								A3($author$project$AG2$viewTab, tab, model.z, false));
						}),
